...
class SpatialAggregator(nn.Module):
    def forward(self, edge_attr, edge_index):
        # Summing up the spatial information (edge attributes) of the neighbors
        row, col = edge_index
        out = torch.zeros_like(edge_attr).scatter_add(0, col.unsqueeze(-1).expand_as(edge_attr), edge_attr)
        return out
...
class MultiHeadGATLayer(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr1):
        ...
        attn_score = (q * k * edge_attr1).sum(dim=-1) / self.out_features ** 0.5
...
class GATGraphSAGE(nn.Module):
    def __init__(self, in_features, edge_features, out_features, num_heads=2, C_dim=16, agg_hidden=768, device=None):
        ...
        self.gat_layer = MultiHeadGATLayer(
            in_features + edge_features, agg_hidden, num_heads, C_dim, device)
    ...
    def forward(self, x, edge_index, edge_attr1):
        ...
        summed_edge_attr1.scatter_add_(0, edge_index_t[:, 1].unsqueeze(-1).expand(-1, encoded_edge_attr1.size(1)),
                                       encoded_edge_attr1)

        # Aggregating edge features
        summed_edge_attr = torch.cat([summed_edge_attr1], dim=1)
        enhanced_x = torch.cat([x, summed_edge_attr], dim=-1)

        # Aggregating neighbor information
        aggregated = self.gat_layer(enhanced_x, edge_index, edge_attr1)
...
def train_one_epoch(
        model, text_decoder, spatial_decoder, data,
        relative_position_tensor, one_hot_targets, optimizer, log_vars_optimizer
):
    model.train()
    text_decoder.train()
    ...
    node_embeddings = model(data.x, data.edge_index, data.edge_position_attr)
    ...
    total_loss = weighted_text_loss + weighted_spatial_loss
    ...
    return total_loss.item(), mse_distance.item(), mse_azimuth.item(), predicted_text_features, node_embeddings
...
if __name__ == "__main__":
    ...
    for epoch in tqdm(range(num_epoch)):
        loss, mse_distance, mse_azimuth, predicted_text_features, node_embeddings = train_one_epoch(
            model, text_decoder, spatial_decoder, data,
            relative_position_tensor, one_hot_targets, optimizer, log_vars_optimizer
        )
        print(f'Epoch [{epoch + 1}/50], Loss: {loss:.4f}, Distance: {mse_distance:.4f}, Azimuth: {mse_azimuth:.4f}')

    torch.save(model.state_dict(), f'pretrained_model_state_{dataset_name}_WO_occ.pth')
    model.eval()
    with torch.no_grad():
        poi_emb = model(data.x, data.edge_index, data.edge_position_attr)
    ...
    model.load_state_dict(torch.load(f'pretrained_model_state_{dataset_name}_WO_occ.pth'))
    model.eval()  # 设置为评估模式
    model = model.to(device)
    with torch.no_grad():
        embeddings = model(pre_trained_data.x, pre_trained_data.edge_index, pre_trained_data.edge_position_attr)
    ...
    out_name = "emb_from_pre_trained_WO_occ.pth"
